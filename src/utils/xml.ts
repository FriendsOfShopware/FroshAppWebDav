export default class XMLBuilder {
    ...
    elem(name: string, value?: string, attributes?: {}): XMLBuilder {
    ...
    private writeElement(element: XMLBuilder): string {
        ...
        // attributes
        const attrs = []
        for (const key in element.attributes) attrs.push(`${key}="${element.attributes[key]}"`)
        if (attrs.length > 0) {
        ...
        // value
        if (element.value) xml.push(element.value)
        // elements
